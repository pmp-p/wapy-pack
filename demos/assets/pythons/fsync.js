...
var urls = { "name":"webcache","id":-1, "index": "/index.html", "err":miss }
...
function awfull_get(url, charset) {
    ...
    function transferFailed(evt) {
        console.log("awfull_get: An error occurred while transferring the file '" + currentTransfer+"'");
        currentTransferSize = -1 ;
    }

    function transferCanceled(evt) {
        console.log("awfull_get: transfer '" + currentTransfer+"' has been canceled by the user.");
        currentTransferSize = -1 ;
    }
    ...
    function transferComplete(evt) {
        if (oReq.status==404){
            ...
            currentTransferSize = oReq.response.length;
            //console.log("awfull_get: Transfer is complete saving : "+ currentTransferSize);
        }
    }
    if (charset)
...
function wasm_file_exists(url, need_dot) {
    ...
    if (need_dot) {
        ...
        if (!url.endsWith('.py')) {
            ...
            //namespace search
            found = url_exists( url + urls.index , 2 )
            //console.log("wasm_([dir]/file)_exists ? :"+url+ ' --> ' + urls.index + " => "+found)
            if (found>0) return found
        }
...
function fget(rel) { return wasm_file_open(rel,rel) }

const exists = wasm_file_exists
const open = wasm_file_open

export { exists, open, awfull_get, fget, currentTransferSize, urls }
